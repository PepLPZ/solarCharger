////////////////PWM SOLAR CHARGE CONTROLLER V-10.1 POSTE 4/////
  //-----------------------------------------------------------------------------
  //
  //  Author: JOSEP LOPEZ SANCHEZ
  //          www.ea3abn.com
  //  This code is for an arduino based Solar PWM charge controller by ea3abn@2010
  //  Credit :logic of 3 stage charging taken from https://github.com/PepLPZ/solarCharger/
  //  Last updated on 10/05/2023
  //  He añadido la deteccion del estado del amplificador ON/OFF en el PIN D1--- 
  //  pendiente de realizar la maniobra tan solo esta definido
  //  Previsto ajustar el valor del sensor de intensidad de consumo de la carga.
  //  Previsto poner una proteccion para desconectar la salida de la bateria hacia la carga, 
  //  cuando el voltaje baje de los 10,2V
  //  40 grados Celsius se toma como temperatura ambiente estándar, debido a la estanqueidad de la caja y
  //  a la exposicion continua al sol,se determina ese valor (Aún en pruebas)
  //------------------------------------------------------------------------------
  ...
  // A4 - LCD SDA 
  // A5 - LCD SCL
  // D0 - NO USADA  puerto serie no usar
  // D1 - NO USADA  puerto serie no usar
  // D2 - Salida de Control del Load MOSFET Q2 
  // D3 - Salida de Control PWM MOSFET Q1 
  // D4 - Push Switch Control LOAD
  // D5 - Battery Red LED       
  // D6 - Battery Green LED 
  // D7 - Battery Blue LED ANULADO PARA USAR LA ENTRADA POR AMPLI/ON/OFF
  // D8 - Load Orange Led 
  // D9 - Load Green Led  
  // D10- ENTRADA DETECCION DE ESTADO DEL AMPLIFICADOR (1 ON)(0 OFF).
  // D11- Solar Green LED  
...
#define BAT_MIN 10.2      // minimum battery voltage for 12V system "CHANGED default BAT_MIN 10.2"
...
#define BAT_RED_LED 5     // LED Rojo indicacion de la bateria
#define BAT_GREEN_LED 6   // LED Verde indicacion de la bateria
////////////#define BAT_BLUE_LED 7    // LED Azul indicacion de la bateria
#define LOAD_ORANGE_LED 8 // LED Naranja indicacion de la salida Carga Desactivada
#define LOAD_GREEN_LED 9  // LED Verde indicacion de la salida Carga Activada
#define AMPLI_PIN 10      // pin-1 is used to control the AMPLI ON/OFF
#define SOL_GREEN_LED 11  // LED Verde indicacion de Entrada de voltaje desde el panel solar
#define ONE_WIRE_BUS 12   // Datos del DS18B20 sensor de temperatura en el pin D12
#define TRANSMISSION_INTERVAL 10000 // Defino el intervalo de tiempo que se enviarán las tramas LoRa 60000=1 Min. 300000=5 min.
//----------------DECLARACION DE TODAS LAS VARIABLES GLOBALES--------------------------
float solar_volt=0;
float bat_volt=0;
float load_current=0;
float solar_current=0;
float offsetVoltage =2.48; // for ACS712 sensor valor anterior 295
float Sensitivity =0.185; // 185mV/A for ACS712-5A variant modificado 9-8-23
int temperature=0;
int temp_change=0;
float system_volt=0;
...
bool sendData = true;   // sendData se usa para enviar datos solo cuando esta variable es true. 
...
bool cargaActivada = true; // Variable global para almacenar el estado de la carga
...
pinMode(AMPLI_PIN ,INPUT); //configuro el pin 1 como entrada de deteccion del ampli ON/OFF
pinMode(LOAD_SWITCH,INPUT); // Define pin D4 input to control the load ON/OFF mediante PULSADOR CARGA o fail safe
...
// Actualizar el estado de los LED indicadores de carga en el inicio
digitalWrite(LOAD_ORANGE_LED, !estadoCarga); // Carga desactivada -> encender LED rojo
digitalWrite(LOAD_GREEN_LED, estadoCarga); // Carga activada -> encender LED verde

}
void loop()
{
 load_Push();         // Lectura del pulsador de la salida LOAD
 load_led();          // Actualizar estado de los LED en función del estado de la carga
 ...
 fail_safe();         // Modo Fail Safe, desconectamos la salida de la carga si la bateria baja por debajo de los 10.5V
 Ampli_Push();        // Deteccion estado amplificador
 
  if (millis() - lastTransmissionTime >= TRANSMISSION_INTERVAL) { // Envía tramas de telemetría sólo si ha pasado un cierto tiempo desde la última transmisión
    sendData = true;                                              // if (millis() - lastTransmissionTime >= TRANSMISSION_INTERVAL) { // envía trama cada 5 minutos (300000 ms)
}                                                                 // print_data();
...
void load_Push(void)
{
  if (digitalRead(LOAD_SWITCH) == HIGH) // Pulsador de carga presionado
  {
    // Encender la carga si está apagada
    if (estadoCarga == 0)
    {
      estadoCarga = 1; // Cambiar estado de carga
      digitalWrite(LOAD_PIN, HIGH); // Encender carga
    }
    // Apagar la carga si está encendida
    else if (estadoCarga == 1)
    {
      estadoCarga = 0; // Cambiar estado de carga
      digitalWrite(LOAD_PIN, LOW); // Apagar carga
    }
...
////////////////////////////////////OBTENER LECTURAS DE SENSADO/////////////////////////////////////////
 void read_data(void) 
 {
   ...
   // Vout=Vin*R2/(R1+R2) => Vin = Vout*(R1+R2)/R2   R1=100 and R2=20
     ...
     load_current = ((read_adc(LOAD_CURRENT_ADC)*0.00488 - offsetVoltage -0.05)/Sensitivity );// VALORES MODIFICADOS PARA INDICAR CORRECTAMENTE valor anterior 0.00727
     //
     //
     solar_current = ((read_adc(SOL_CURRENT_ADC)*0.00488 - offsetVoltage -0.05)/ Sensitivity );//VALORES MODIFICADOS PARA INDICAR CORRECTAMENTE valor anterior 0.00724
      if (load_current <0)
     {
      load_current = 0;
     }
...
void print_data(void) 
{
  ...
  Serial.print("AirVoice1/");
  Serial.print("poste4");
  Serial.print("/loadAMP/");
  Serial.println(load_current);
   delay(1400); 
  // Envia el status del Amplificador de audio
  Serial.print("AirVoice1/");
  Serial.print("poste4");
  Serial.print("/statAMP/");
  Serial.println(estadoAmpli); 
 }
...
////////////////////////////////////CHARGE SET POINT //////////////////////////////////////////////
 void setpoint(void)
{
  temp_change =temperature-40.0; // 40 grados Celsius se toma como temperatura ambiente estándar
...
//--------------------------------------------------------------------
/////FAIL SAFE DESCONEXION DE LA SALIDA A LA CARGA CUANDO 
///  LA BATERIA ESTÉ ENTRE (10,2V Y 12,5V)                    ////
//--------------------------------------------------------------------
void fail_safe(void)
{
    if (bat_volt >= 12.5 && !cargaActivada)  // Conectar carga si el voltaje es mayor o igual a 12.7V y la carga está desactivada
    {
        digitalWrite(LOAD_PIN, HIGH);
        cargaActivada = true;  // Actualizar el estado de la carga
    }
    else if (bat_volt <= 10.2 && cargaActivada)  // Desconectar carga si el voltaje es menor o igual a 10.2V y la carga está activada
    {
        digitalWrite(LOAD_PIN, LOW);
        cargaActivada = false;  // Actualizar el estado de la carga
    }
}
