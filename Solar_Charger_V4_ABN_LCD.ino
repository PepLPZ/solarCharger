...
//float Sensitivity =0.66; // 66mV/A for ACS712-30A variant
int temperature=0;
int temp_change=0;
float system_volt=0;
...
void loop()
{
 ...
 //load_control();          //control the load<<<<<< COMENTANDO ESTE CODIGO ANULAMOS EL PULSADOR >>>>>>>
 led_indication();        // led indication
 print_data();            // print in serial monitor
 lcd_display();           // lcd display
 //load_Push();           // Lectura del pulsador    <<<<<< COMENTANDO ESTE CODIGO ANULAMOS EL PULSADOR >>>>>>>
 }
//************************************************************ PROGRAM END *************************************************
...
//-----------------------------------------------------------------------------------------------------
/////////////////////////////////////////////LOAD CONTROL SWITCH ON/OFF BY PEP/////////////////////////
//-----------------------------------------------------------------------------------------------------  
// void load_Push(void)
//{  
...
//  }
//}
////////////////////// INVERTIDO EL MODO DE TRABAJO ////////////////////////////////////////////
// void load_Push(void)
//{  
  // Encender la carga si pulsamos el pulsador y la variable Biestable es igUal a cero  
//  {
// if (digitalRead(LOAD_SWITCH)==LOW && Biestable==1 && load_status==1)
//  {
//   Biestable=0;
//   load_status=0;  
//   digitalWrite(LOAD_PIN, LOW); // load is OFF
...
//  }
  // Apagar LED si pulsamos el pulsador y la variable biestable es igUal a uno
//   else if (digitalRead(LOAD_SWITCH)==LOW && Biestable==0 && load_status==0)
//  {
//   Biestable=0;// Cambiamos valor de variable a 1
//   load_status=1; 
//   digitalWrite(LOAD_PIN, HIGH); //load is ON
//   delay(10);
//  }
...
//------------------------------------------------------------------------------------------------------------
  void print_data(void) 
  {
    ...
    //Serial.print((PWM_Duty /255 )*100);  // convert a number between 0 -1024  to  0-1024 then convert float to integer 
    //Serial.print("%");
